...
> After some time, I managed to get the middle challenge moveie over done with a ok result. It struggles in the middle there because of sudden change in light contitions, so I turned up `hist_frames` to about 30, and it didn't bother that it diden't found lanes (because of the memory). There was one more challenge [video](https://github.com/KvalheimRacing/CarND/blob/master/P1_Lane_Lines/test_videos_output/challenge18.gif) (not for this project, but for the advanced lane lines project in the end of may), where the algoritm had no chance, I will talk about this in the reflections below.
